classdef PM_count < ALGORITHM
    ...
    methods
        function main(Algorithm,Problem)
            ...
            csvFilename = sprintf('/Users/azumayuki/Documents/LONs/data09-20-pre/RWMOP%d_count.csv',pn);
            ...
            while Algorithm.NotTerminated(Population)
                ...
                for i = 1:N
                    ...
                    if stableCount(i) < stableThreshold
                        ...
                        CVM = Count(Candidates);
                        [mi, ~] = min(CVM);
                        bestIdx = find(CVM==mi);
                        CV_values = arrayfun(@(x) sum(max(0,x.cons)), Candidates);
                        CV_values = CV_values(bestIdx);
                        [~, best] = min(CV_values);
                        Offspring(i) = Candidates(bestIdx(best));
                    else
                        % 安定状態なら突然変異なしでそのまま
                        Offspring(i) = Parent;
                    end
                ...
                if all(stableCount >= stableThreshold)
                    ...
                    fprintf("count:実行可能解割合：%f 平均到達世代:%f\n",ratio,mean_gen)
                    break;
                end
                PrevPopulation = Population;
            end
