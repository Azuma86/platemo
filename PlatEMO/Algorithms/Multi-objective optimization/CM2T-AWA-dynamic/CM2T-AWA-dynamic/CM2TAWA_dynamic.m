classdef CM2TAWA_dynamic < ALGORITHM
...
% rate_evol          ---  0.5 --- Ratio of iterations to evolve with only MOEA/D
% rate_unvec                ---  0.5 --- Rate of unnecessary vector
    ...
    methods
        function main(Algorithm,Problem)
           %% Parameter setting
            ...
            [S, type,rate_evol,rate_unvec,evaluation] = Algorithm.ParameterSet(20,3,0.5,0.5,1);
            ...
            %% vector adjustment parameter
            % number of update vector
            last_gen = ceil(Problem.maxFE/Problem.N/5);
            current_sca = zeros(last_gen,Problem.N);
            current_con = zeros(last_gen,Problem.N);
            current_sca_con = zeros(last_gen,Problem.N);
            gen_count = 0;
            c = 0;
            ...
            for i = 1 : Problem.N
                ...
                [SubPopulation(i,:),SubPop_Constraint_value(i,:),SubPop_Scalarizing_value(i,:)] = EnSele(UpdatePopulation,Sum_Constraint,Scalarizing_value,S);
            end
            ...
            while Algorithm.NotTerminated(Population)
                ...
                for i = 1 : Problem.N
                    ...
                    [SubPopulation(i,:),SubPop_Constraint_value(i,:),SubPop_Scalarizing_value(i,:)] = EnSele([SubPopulation(i,:),Offspring],Update_Sum_Constraint,Update_Scalarizing_value,S);
                end
                current_sca = bestscalar(SubPop_Scalarizing_value,current_sca,gen,last_gen);
                current_con = bestcon(SubPop_Constraint_value,current_con,gen,last_gen);
                current_sca_con = bestscalar_con(SubPop_Scalarizing_value,SubPop_Constraint_value,current_sca_con,gen,last_gen);
                change_ind = calc_maxchange(current_sca,current_con,current_sca_con,last_gen,c);
                ...
                if Problem.FE > rate_evol*Problem.maxFE
                    ...
                    if change > rate_unvec && ~isempty(EP) && gen_count > 20
                        %disp("a")
                        gen_count = 0;
                        [~,ia,~] = unique(EP.objs,'rows');
                        EP = EP(ia);
                        [Population,SubPopulation,SubPop_Constraint_value,SubPop_Scalarizing_value,Del,W] = updateWeight_dynamic(Population,SubPopulation,SubPop_Constraint_value,SubPop_Scalarizing_value,W,Z,EP,change_ind);
                        n = length(find(Del));
                        %% 初期個体を Problem.N 個の初期サブ個体群に収容
                        UpdatePopulation = Population;
                        Sum_Constraint = sum(max(0,Population.cons),2);
...
function change_ind = calc_maxchange(current_sca,current_con,current_sca_con,last_gen,c)
    change_sca = current_sca(1,:)-current_sca(last_gen,:);
    change_con = current_con(1,:)-current_con(last_gen,:);
    current_sca_con = current_sca_con(1,:)-current_sca_con(last_gen,:);
    change_ind = find(change_sca<=c & change_con<=c & current_sca_con<=c);
end
