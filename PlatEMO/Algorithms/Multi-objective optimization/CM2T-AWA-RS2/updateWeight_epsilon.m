function [Population,SubPopulation,SubPop_Constraint_value,SubPop_Scalarizing_value,Del,W] = updateWeight_epsilon(Population,SubPopulation,SubPop_Constraint_value,SubPop_Scalarizing_value,W,Z,EP,nus,epsilon)
...
%--------------------------------------------------------------------------
 ...
 %% Update the current population by EP
    ...
    feasible_index = find(sum(max(0,Population.cons),2) <= epsilon);
