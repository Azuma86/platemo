classdef CM2TAWA_scalr < ALGORITHM
    ...
    methods
        function main(Algorithm,Problem)
           %% Parameter setting
            ...
            [S, type,rate_update_weight,rate_evol,wag,evaluation] = Algorithm.ParameterSet(20,3,0.05,0.8,100,1);
            ...
            for i = 1 : Problem.N
                ...
                [SubPopulation(i,:),SubPop_Constraint_value(i,:),SubPop_Scalarizing_value(i,:)] = EnSele_scalr(UpdatePopulation,Sum_Constraint,Scalarizing_value,S);
            end
            ...
            while Algorithm.NotTerminated(Population)
                ...
                for i = 1 : Problem.N
                    ...
                    Nadir = max(Nadir,Offspring(Offspring_gennum).objs);
                    Z = min(Z,Offspring(Offspring_gennum).objs);
                ...
                for i = 1 : Problem.N
                    ...
                    [SubPopulation(i,:),SubPop_Constraint_value(i,:),SubPop_Scalarizing_value(i,:)] = EnSele_scalr([SubPopulation(i,:),Offspring],Update_Sum_Constraint,Update_Scalarizing_value,S);
                end
                [a,b] = min(SubPop_Scalarizing_value(1,:));
                current_sca = bestscalar2(SubPop_Scalarizing_value,current_sca,gen,last_gen);
                current_con = bestcon2(SubPop_Constraint_value,current_con,gen,last_gen);
                current_sca_con = bestscalar_con2(SubPop_Scalarizing_value,SubPop_Constraint_value,current_sca_con,gen,last_gen); 
                ...
                if Problem.FE > rate_evol*Problem.maxFE
                    ...
                    if ~mod(ceil(Problem.FE/Problem.N),wag/5) && ~isempty(EP)
                        ...
                        for i = 1 : n
                            ...
                            % スカラー化関数と制約違反量を目的関数値とする個体群を作成．
                            [SubPopulation(Problem.N-n+i,:),SubPop_Constraint_value(Problem.N-n+i,:),SubPop_Scalarizing_value(Problem.N-n+i,:)] = EnSele_scalr(UpdatePopulation,Sum_Constraint,Scalarizing_value,S);
                        end
                    end
                end
                 % サブ個体群を1つの個体群に集約．
                ...
                if Problem.FE >= Problem.maxFE
                    ...
                    writematrix(current_sca,'current_sca.txt','WriteMode','append')
                    writematrix(current_sca_con,'current_sca_con.txt','WriteMode','append')
                    writematrix(current_con,'current_con.txt','WriteMode','append')
                end
            end
        end
    end
