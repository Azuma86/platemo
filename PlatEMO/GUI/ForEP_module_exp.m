...
%--------------------------------------------------------------------------
    ...
    methods(Access = private)
                ...
                for p = 1 : length(PRO)
                    for a = 1 : length(ALG)
                        ...
                        elseif ~isempty(runIndex)
                            try
                                Future = arrayfun(@(s)parfeval(@parallelFcn,3,ALG(a),PRO(p)),runIndex);
                                while ~all([Future.Read])
                                    ...
                                    [r,result,metric,EP] = fetchNext(Future,0.01);
                                    if ~isempty(r)
                                        obj.ResultSave(p,a,runIndex(r),result,metric);
                                        obj.EPSave(p,a,runIndex(r),EP);
        ...
        %% Save the result file
        function EPSave(obj,p,a,r,EP)
            folder   = fullfile(obj.data.folder,class(obj.data.ALG(a)));
            [~,~]    = mkdir(folder);
            filename = fullfile(folder,sprintf('%s_%s_EP_M%d_D%d_%d.mat',class(obj.data.ALG(a)),class(obj.data.PRO(p)),obj.data.PRO(p).M,obj.data.PRO(p).D,r));
            save(filename,'EP');
        end
...
function [result,metric,EP] = parallelFcn(Algorithm,Problem)
