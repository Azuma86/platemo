function [fig] = criticaldifference_new(s,labels,alpha,SortOrder)
%
...
line([0 0 0 cd/(k-1) cd/(k-1) cd/(k-1)], [121 116 119 119 116 121], 'LineWidth', 3, 'Color', 'r');
% line([0 0 0 cd/(k-1) cd/(k-1) cd/(k-1)], [127 123 125 125 123 127], 'LineWidth', 3, 'Color', 'r');
h = text(0.5*cd/(k-1), 129, ['{\color{red}Critical Distance} = ' num2str(cd,3)], 'FontSize', 16, 'HorizontalAlignment', 'center');
% h = text(0.5*cd/(k-1), 131, ['Critical Distance = ' num2str(cd,3)], 'FontSize', 16, 'HorizontalAlignment', 'center');


% text(0.03, 116, ['Critical Distance=' num2str(cd)], 'FontSize', 12, 'HorizontalAlignment', 'left', 'Color', 'r');

for i=1:k
   text((i-1)/(k-1), 110, num2str(k-i+1,3), 'FontSize', 14, 'HorizontalAlignment', 'center');
end
...
for i=1:ceil(k/2)
   line([(k-r(i))/(k-1) (k-r(i))/(k-1) 1.2-fix2], [100 100-fix-5*(n+1)-10*i 100-fix-5*(n+1)-10*i], 'LineWidth', 1, 'Color', 'k');
   h = text(1.2-fix2, 102-fix - 5*(n+1)- 10*i + 2, num2str(r(i),4), 'FontSize', 14, 'HorizontalAlignment', 'right');

   text(1.25-fix2, 100-fix - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 16, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
...
for i=ceil(k/2)+1:k
   ...
   line([(k-r(i))/(k-1) (k-r(i))/(k-1) -0.2+fix2], [100 100-fix-5*(n+1)-10*(k-i+1) 100-fix-5*(n+1)-10*(k-i+1)], 'LineWidth', 1, 'Color', 'k');

   text(-0.2+fix2, 102-fix - 5*(n+1) -10*(k-i+1)+2, num2str(r(i),4), 'FontSize', 14, 'HorizontalAlignment', 'left');

   text(-0.25+fix2, 100-fix - 5*(n+1) -10*(k-i+1), labels{idx(i)}, 'FontSize', 16, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right');
...
for i=1:size(clique,1)
   ...
   line([((k-min(R))/(k-1)) ((k-min(R))/(k-1)) ((k-min(R))/(k-1)) ((k - max(R))/(k-1)) ((k - max(R))/(k-1)) ((k - max(R))/(k-1))], [100+1-5*i 100-1-5*i 100-5*i 100-5*i 100-1-5*i 100+1-5*i], 'LineWidth', 3, 'Color', 'r');
end
